...
En effet, les catalogues brassent une somme extrêmement importantes de « métadonnées », qui sont en réalité des données  parfois recherchées pour elles-mêmes, mais actuellement sous-exploitées car mal liées entre elles. En ce sens, la transition bibliographique s'inscrit très nettement dans la constitution et l'horizon d'un [**web sémantique**](https://www.w3.org/2001/sw/), notion inventée par Tim Berners-Lee[^7] (considéré comme le principal inventeur du web) et notamment développée avec James Hendler et Ora Lassila en 2001 dans [un article fondateur](https://www.lassila.org/publications/2001/SciAm.pdf). Le web sémantique « repose sur l'établissement de **liens directs et signifiants**, c'est-à-dire interprétables par une machine, de (méta)donnée à (méta)donnée au sein d'un "**web de données**" » (Philippe le Pape[^8]). Il s'agirait en fait de permettre aux machines de comprendre la sémantique, d'avoir accès au sens de l'information disponible sur le web. Le **web de données** renvoie au projet de relier et structurer les données du web pour former un réseau global d'information. 
